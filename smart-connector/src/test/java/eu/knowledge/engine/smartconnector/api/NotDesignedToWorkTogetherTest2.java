 ...
 *
 */
public class NotDesignedToWorkTogetherTest2 {

	private static final Logger LOG = LoggerFactory.getLogger(NotDesignedToWorkTogetherTest2.class);
	...
	public void test() throws InterruptedException {
		...
		// to test whether the reasoner of this example still works, we slightly modify
		// a domain rule where it does not produce a binding set. This should not cause
		// this unit test to fail, because the other domain rule is all we need.
		HashSet<TriplePattern> rule2ant = new HashSet<>(Arrays.asList(
				new TriplePattern(
						"<lamp3> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://example.org/OnOffLamp>"),
				new TriplePattern(
						"<lamp3> <http://example.org/isOn> \"false\"^^<http://www.w3.org/2001/XMLSchema#boolean>")));
		HashSet<TriplePattern> rule2con = new HashSet<>(Arrays.asList(
				new TriplePattern(
						"<lamp3> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://example.org/DimmableLamp>"),
				new TriplePattern("<lamp3> <http://example.org/hasBrightness> \"0\"")));
	...
	public static void close() {
		LOG.info("Clean up: {}", NotDesignedToWorkTogetherTest2.class.getSimpleName());
		try {
			kn.stop().get();
