...
public class BindingValidator {
	...
	/**
	 * Throws an {@link InvalidArgumentException} if one of the bindings uses a key
	 * that is not one of the graph pattern's variables.
	 *
	 * 
	 * @param pattern
	 * @param bindings
	...
	public void validatePartialBindings(GraphPattern pattern, BindingSet bindings) {
		...
		Set<Var> variables = pattern.getVariables();
		bindings.forEach(b -> {
			b.forEach((k, v) -> {
				if (!variables.contains(k)) {
					throw new IllegalArgumentException(String.format(
							"Given binding set uses key '%s', but this does not occur as a variable in graph pattern '%s'.",
							k, pattern.getPattern()));
				}
			});
		});
	}
	...
	public void validateCompleteBindings(GraphPattern pattern, BindingSet bindings) {
		...
		Set<Var> variables = pattern.getVariables();
		bindings.forEach(b -> {
			var notSeenYet = new HashSet<Node_Variable>(variables);
			b.forEach((k, v) -> {
				if (!variables.contains(k)) {
					throw new IllegalArgumentException(String.format(
							"Given binding set uses key '%s', but this does not occur as a variable in graph pattern '%s'.",
			...
			if (!notSeenYet.isEmpty()) {
				throw new IllegalArgumentException(String.format(
						"Expected a complete binding, but was missing some variable(s) that ARE in the graph "
								+ "pattern ('%s'), but are missing from the binding, namely: %s. Note that the question mark should NOT be included in your binding keys.",
						pattern.getPattern(), notSeenYet.toString()));
			}
		});
	}

	public void validateIncomingOutgoingAnswer(GraphPattern pattern, BindingSet incoming, BindingSet outgoing) {
		// make sure each of the outgoing bindings 'fits' on at least 1 incoming
		// binding.
		...
		outgoing.forEach(outgoingBinding -> {
			// Check if there is an incoming binding that is matches (is a 'sub-binding' of)
			// the outgoing binding.
			if (incoming.stream().allMatch(incomingBinding -> !incomingBinding.isSubBindingOf(outgoingBinding))) {
				// If not, it is invalid.
				throw new IllegalArgumentException("KB gave outgoing binding " + outgoingBinding
						+ ", but this doesn't have a matching incoming binding!");
			}
		});
	}

	public void validateIncomingOutgoingReact(GraphPattern argumentPattern, GraphPattern resultPattern,
			BindingSet incoming, BindingSet outgoing) {
		...
		if (resultPattern == null && outgoing != null && outgoing.size() > 0) {
			...
			outgoing.forEach(outgoingBinding -> {
				...
				if (incoming.stream().map(incomingBinding -> incomingBinding.keepOnly(overlappingVariables))
						.allMatch(b -> !b.isSubBindingOf(outgoingBinding))) {
					throw new IllegalArgumentException("KB gave outgoing binding " + outgoingBinding
							+ ", but this doesn't have a matching incoming binding!");
				}
			});
