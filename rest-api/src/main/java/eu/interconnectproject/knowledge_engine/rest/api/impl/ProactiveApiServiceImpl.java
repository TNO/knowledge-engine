...
public class ProactiveApiServiceImpl extends ProactiveApiService {
	...
	private RestKnowledgeBaseManager store = RestKnowledgeBaseManager.newInstance();
		...
		@Override
		public Response scAskPost( @NotNull String knowledgeBaseId,  @NotNull String knowledgeInteractionId, List<Map<String, String>> bindings, SecurityContext securityContext) throws NotFoundException {
			...
			try {
				...
				return Response.status(500).entity("Something went wrong while sending a POST or while waiting on the REACT.").build();
			}
			return Response.ok().entity(ar).build();
		}

		@Override
		public Response scPostPost( @NotNull String knowledgeBaseId,  @NotNull String knowledgeInteractionId, List<Map<String, String>> bindings, SecurityContext securityContext) throws NotFoundException {
			var kb = this.store.getKB(knowledgeBaseId);
			if (kb == null) {
				return Response.status(404).entity("Smart connector not found, because its ID is unknown.").build();
			}
