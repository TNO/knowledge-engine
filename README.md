...
To start a new instance of the REST API knowledge engine version 1.0.2, make sure you have `git checkout 1.0.2` the tag `1.0.2`. Now make sure you run the `mvn clean install` command successfully from the root of the repository.
...
```bash
java -Dorg.slf4j.simpleLogger.logFile=ke.log -cp "smart-connector-rest-dist-1.0.2.jar:dependency/*" eu.knowledge.engine.rest.Main 8280
```
...
```bash
nohup java -cp "smart-connector-rest-dist-1.0.2.jar:dependency/*" eu.knowledge.engine.rest.Main 8280 > ke.log
```
...
java -Dorg.slf4j.simpleLogger.logFile=kd.log -cp "knowledge-directory-1.0.2.jar:dependency/*" eu.knowledge.engine.knowledgedirectory.Main 8080
```
...
4. Build and push the new docker images:
	- `docker build ./smart-connector-rest-dist -t docker-registry.inesctec.pt/interconnect/knowledge-engine/smart-connector-rest-dist:1.0.2`
	- `docker build ./knowledge-directory -t docker-registry.inesctec.pt/interconnect/knowledge-engine/knowledge-directory:1.0.2`
	- `docker push docker-registry.inesctec.pt/interconnect/knowledge-engine/smart-connector-rest-dist:1.0.2`
	- `docker push docker-registry.inesctec.pt/interconnect/knowledge-engine/knowledge-directory:1.0.2`
