...
To start a new instance of the REST API knowledge engine version 1.0.1, make sure you have `git checkout 1.0.1` the tag `1.0.1`. Now make sure you run the `mvn clean install` command successfully from the root of the repository.
...
```bash
java -Dorg.slf4j.simpleLogger.logFile=ke.log -cp "smart-connector-rest-dist-1.0.1.jar:dependency/*" eu.knowledge.engine.rest.Main 8280
```
...
```bash
nohup java -cp "smart-connector-rest-dist-1.0.1.jar:dependency/*" eu.knowledge.engine.rest.Main 8280 > ke.log
```
...
java -Dorg.slf4j.simpleLogger.logFile=kd.log -cp "knowledge-directory-1.0.1.jar:dependency/*" eu.knowledge.engine.knowledgedirectory.Main 8080
```
